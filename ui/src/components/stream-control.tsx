  ...
  const getStreamUrl = async (): Promise<string | null> => {
    try {
      ...
      if (!response.ok) {
        ...
        throw new Error(
          errorData.error || `Failed to get stream token: ${response.status}`,
        );
      }
  ...
  const openStreamWindow = async () => {
    try {
      ...
      const newWindow = window.open(
        streamUrl,
        "ComfyStream OBS Capture",
        "width=1024,height=1024",
      );

      if (!newWindow) {
        throw new Error(
          "Failed to open stream window. Please check your popup blocker settings.",
        );
      }
    } catch (error) {
      ...
      alert(
        error instanceof Error
          ? error.message
          : "Failed to open stream window. Please try again.",
      );
    } finally {
      setIsLoading(false);
    }
