...
async def cache_control_middleware(request, handler):
    ...
    return response
# Check if we're running as vanilla custom node (PromptServer available)
try:
    from server import PromptServer
    # Check if PromptServer has instance and if it's initialized
    if hasattr(PromptServer, 'instance') and PromptServer.instance is not None:
        if hasattr(PromptServer.instance, 'routes'):
            routes = PromptServer.instance.routes
            logging.info("[ComfyStream API] Running as vanilla custom node - PromptServer available")
        else:
            routes = None
            logging.info("[ComfyStream API] PromptServer.instance.routes not available yet")
    else:
        routes = None
        logging.info("[ComfyStream API] PromptServer.instance not available yet - may be package mode")
except (ImportError, AttributeError) as e:
    # Running as package mode - no PromptServer
    routes = None
    logging.info(f"[ComfyStream API] Running as package mode - no PromptServer available: {e}")

# Get the path to the static build directory (always needed)
...
logging.info(f"[ComfyStream API] Extension name: {EXTENSION_NAME}, Static route: {STATIC_ROUTE}")
logging.info(f"[ComfyStream API] Checking routes setup - routes: {routes is not None}")
if routes is not None:
        # Add static route for Next.js build files using the dynamic extension name
        logging.info(f"[ComfyStream API] Setting up static route: {STATIC_ROUTE} -> {STATIC_DIR}")
        ...
        logging.info("[ComfyStream API] Registering /comfystream/extension_info route")
