...
def setup_opencv_cuda():
    ...
    try:
        ...
        if hasattr(cv2, 'cuda') and cv2.cuda.getCudaEnabledDeviceCount() > 0:
            logger.critical("OpenCV CUDA already installed and working")
            return True
    except ImportError:
        pass
    ...
    try:
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            if extracted_dir.exists():
                target_dir = workspace_dir / "opencv-cuda" / "build"
...
def cli_main():
    """
    Command-line interface entry point for opencv-cuda setup.
    Returns:
        int: 0 for success, 1 for failure
    """
    success = setup_opencv_cuda()
