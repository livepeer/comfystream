...
class Pipeline:
    ...
    async def put_audio_frame(self, frame: av.AudioFrame, trickle: bool = False):
        ...
        frame.side_data.input = self.audio_preprocess_trickle(frame) if trickle else self.audio_preprocess(frame)
        frame.side_data.skipped = True
    ...
    def audio_preprocess_trickle(self, frame: av.AudioFrame) -> Union[torch.Tensor, np.ndarray]:
        """Preprocess an audio frame before processing.
