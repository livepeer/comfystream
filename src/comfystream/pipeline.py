...
class Pipeline:
    ...
    def audio_preprocess(self, frame: av.AudioFrame) -> Union[torch.Tensor, np.ndarray]:
        ...
        if audio_data.ndim == 1:
            ...
            # Multi-channel audio - determine layout based on shape
            if audio_data.shape[0] <= audio_data.shape[1]:
                # Channel-first format [channels, samples]
                if audio_data.shape[0] == 1:
                    # Single channel in 2D array [1, samples] - flatten to 1D
                    # Used by RTC frames for Whisper (TODO: use a node for conversions)
                    return audio_data.ravel().reshape(-1, 2).mean(axis=1).astype(np.int16)
                else:
                    # Multi-channel audio [N, samples] - average all channels to mono
                    # Used by the trickle Frame Processor for Whisper (TODO: use a node for conversions)
                    processed_audio = audio_data.mean(axis=0)
        
        # Normalize float values to int16 range
        if processed_audio.dtype in [np.float32, np.float64]:
            max_abs_val = np.abs(processed_audio).max()
            if max_abs_val <= 1.0:
