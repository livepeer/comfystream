...
This module defines ComfyUI-specific BaseModel classes for validating and
serializing API request parameters. Core streaming models are imported from
pytrickle.
"""
...
class ComfyUIParams(BaseModel):
    ...
    def merge_with_defaults(
        ...
        current_width: int = DEFAULT_WIDTH,
                ...
                if isinstance(item, str):
                    try:
                        ...
                        if isinstance(parsed, dict):
                            ...
                            raise ValueError(
                                "Each JSON string in prompts must parse to a dictionary"
                            )
                    ...
                    raise ValueError(
                        "Each item in prompts list must be either a JSON string or dict"
                    )
            return result
        ...
        if isinstance(v, str):
            try:
                ...
                if isinstance(parsed, dict):
                    ...
                    raise ValueError(
                        "Provided JSON string must parse to a dictionary or array of dictionaries"
                    )
            except json.JSONDecodeError:
                raise ValueError("Provided prompt string must be valid JSON")

        raise ValueError(
            "Prompts must be either a JSON string, dictionary, or list of JSON strings/dictionaries"
        )
