...
def log_comfystream_error(
    exception: Exception, logger: Optional[logging.Logger] = None, level: int = logging.ERROR
) -> None:
    """
...
class ComfyStreamInputTimeoutError(Exception):
    ...
    def __init__(
        self, input_type: str, timeout_seconds: float, details: Optional[Dict[str, Any]] = None
    ):
    ...
    def get_log_details(self) -> Dict[str, Any]:
        ...
        base_details = {"input_type": self.input_type, "timeout_seconds": self.timeout_seconds}
...
class ComfyStreamTimeoutFilter(logging.Filter):
    ...
    def filter(self, record):
        ...
        try:
            ...
            if (
                "ComfyStreamAudioBufferError" in message
                or "ComfyStreamInputTimeoutError" in message
            ):
                return False
            ...
            if record.exc_info and record.exc_info[1]:
                ...
                if (
                    "ComfyStreamAudioBufferError" in exc_str
                    or "ComfyStreamInputTimeoutError" in exc_str
                    or "ComfyStreamAudioBufferError" in exc_type
                    or "ComfyStreamInputTimeoutError" in exc_type
                ):
                    return False
